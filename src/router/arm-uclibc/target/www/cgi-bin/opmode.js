...
function getkey(type, e)
{
	...
	else if(type == "shareName")// not / * ? " <> \ : 
	{
        ...
        else if(type == "mediaServerName")// not / * ? " <> \ :
        {
			...
			alert("眤ぃ?  ':'? ReadyDLNA 碈蔨狝?竟?");  // || (keycode==58)
                        return false;
		}	
                else
                        return true;
...
function checkpsk(passphrase, passphrase_len)
{
	...
	if ( len == 64 )
	{
		for(i=0;i<len;i++)
		{
			...
			{
				alert("盞?パ8~63?ASCII才┪64?せ???Θせ???┮ボ才ぇ0, 1, 2, ..., 8, 9, A, B, C, D, E, F");
				return false;
			}
		}
	}
...
var wep_128="盞??\n\n盞???? 26 せ?0  9 の A  F才┪ 13  ASCII 才";
var wep_64="盞??\n\n盞???? 10 せ?0  9 の A  F才┪ 5  ASCII 才";
...
function sAlert(str, callback_ok, callback_cancel, dwidth, anc){
	...
	var anc_v=anc || 0; // ancㄩ click apply button, not close div
