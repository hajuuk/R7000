...
#define EMF_MCAST_IN_UCAST					1
#else
#undef EMF_MCAST_IN_UCAST
#endif

#if defined(EMF_MCAST_IN_UCAST)
/* The Multicast modes */
#define EMF_MCAST_MODE_NORMAL				0
#define EMF_MCAST_MODE_BY_UCAST			1

/* Max hosts supported to send multicast in unicast */
#define EMF_MAX_HOSTS_PER_MGRP			16
#define EMF_MAX_HOSTS_PER_IFACE			16
#endif
...
typedef struct emfc_snooper
{
	...
	int32   (*input_fn)(struct emfc_snooper *snooper, void *ifp,
	                    uint8 *iph, uint8 *igmph, int rt_port, uint8 *mh_mac);
#else
	int32   (*input_fn)(struct emfc_snooper *snooper, void *ifp,
	...
	#if defined(EMF_MCAST_IN_UCAST)
	/* Function called when EMF is disabled */
	int32   (*gethosts_fn)(struct emfc_snooper *snooper, void *ifp, uint32 mgrp_ip, 
...
extern uint32 emfc_input(struct emfc_info *emfc, void *sdu, void *ifp,
                         uint8 *iph, int rt_port, uint8 *hmac);
#else
extern uint32 emfc_input(struct emfc_info *emfc, void *sdu, void *ifp,
...
#if defined(EMF_MCAST_IN_UCAST)
/*
 * Description: This function returns the Host Source MACs mhmaclist who are members
 *              of mgrp_ip within the interface ifp 
 *
 * Input:       emfc    - EMFL Common code global instance handle
 * Input:       mhmac_size - the buffer size of mhmaclist.
 * Output       mhmaclist - The Host's MAC address.
 *
 * Return:      < 0: error, phmac and pmacaddress are not updated
 *              >= 0: number of mac address written to the returned in mhmaclist.
 */
int
emfc_gethostmaclist(struct emfc_info *emfc,
	void *ifp,
	uint32 mgrp_ip,
	int mhmac_size,
	uint8 *mhmaclist);
