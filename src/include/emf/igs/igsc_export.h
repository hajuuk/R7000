...
/*
 * Description: This functions generates an IGMP Query and send to the  
 *              host in interface dev by unicast. It is used to determine
 *               the group members on the attached dev ports.
 * Input:       dev  - the interface.
 * Input:       hmac - the MAC address of the particular host.
 *
 * Return:      SUCCESS or FAILURE
 */
extern int32 igsc_igmp_query_sendto(struct igsc_info *igsc_info, void *dev, uint8 *hmac);
