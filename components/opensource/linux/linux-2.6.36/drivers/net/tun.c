...
static int run_filter(struct tap_filter *filter, const struct sk_buff *skb)
{
	...
	int i;
    /* Foxconn Bob added start on 11/25/2014, workaround to avoid kernel panic if OpenVPN and IPv6 are both enabled,
       need to be removed when OpenVPN supports IPv6 */
    if(eh->h_proto== htons(0x86DD))
    {
        //printk(KERN_INFO"Drop IPv6 packets\n");
        return 0;
    }
    else
        return 1;
    /* Foxconn Bob added end on 11/25/2014 */
